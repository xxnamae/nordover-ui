...
- **Token artifacts are generated**: `tokens-*.json` (DTCG) **and** `docs/handoff/nordover-elementor-*.json` (Elementor export) are both derived from the token CSS (CSS is the single source of truth — never hand-edit the JSON). Any token value change must regenerate **both** in the same commit: `npm run build:derived` (= `build:tokens` + `build:elementor`). CI enforces sync via `npm run check:tokens` **and** `npm run check:elementor` (run both with `npm run check:all`). The `.githooks/pre-commit` hook does this automatically when token CSS is staged — it is activated by the `prepare` script on `npm install` (or manually: `git config core.hooksPath .githooks`). See `docs/wiki/decisions/2026-06-01-json-token-eksport-dtcg.md`
